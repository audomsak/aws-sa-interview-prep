...
The OpenShift architecture page covered the platform-wide picture — Operators, the Cluster Version Operator, node roles. This page zooms into three specific mechanisms you're very likely to get drilled on individually, since they map directly onto your resume experience: how OpenShift locks down what a pod can request (SCC), how it packages operational knowledge as software (Operators, revisited with a concrete example), and how it exposes services externally (Routes vs Ingress).
